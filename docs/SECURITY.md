...
9. **Admin Actions Are Out of Scope:** Vulnerabilities that require an administrator to actively perform unsafe actions are **not considered valid vulnerabilities**. **Admins have full system control and are expected to understand the security implications of their actions and configurations**. This includes but is not limited to: adding malicious external model or MCP servers, registering untrusted local MCP packages, or intentionally weakening security settings. **Reports requiring admin negligence or social engineering of admins may be rejected.**
...
10. **MCP Package Execution Is Intended Behavior:** Managed local MCP packages execute administrator-installed code in separate supervised processes. The Functions administration UI is removed from this fork, and MCP is the supported model-callable extension mechanism. Local packages and remote MCP servers must be isolated and permissioned according to the managed MCP runtime documentation.

> [!IMPORTANT]
> **For administrators:** Treat local MCP package installation and privileged MCP package configuration as root-equivalent access. Only grant those capabilities to administrators you trust with direct access to your server.
