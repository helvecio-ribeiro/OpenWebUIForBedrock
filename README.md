...
A proposed [Qwen MCP Adapter](docs/qwen-mcp-adapter.md) is documented as a future, independent GitHub project. It would expose Qwen through an OpenAI-compatible endpoint while owning MCP discovery and tool execution externally. Its implementation intentionally does not belong in Lambda WebUI.
...
- [Amazon Linux 2023 on AWS EC2](docs/install-amazon-linux-ec2.md) — production source build, IAM/Bedrock, systemd, managed MCP services, and HTTPS boundaries.
- [Raspberry Pi OS 64-bit](docs/install-raspberry-pi.md) — ARM64 source build, Pi resource constraints, systemd, managed MCP services, and remote Voice Mode requirements.

Both guides install this customized fork rather than the upstream Open WebUI package. Pin a known release or commit, preserve the application and MCP data directories, and do not apply upstream upgrades without reviewing this fork's changes.
